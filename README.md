...
The contact form at `/contacto` uses `mailto:` and opens the user's preferred email client.

- Recipient: `info@digio.es`
- Subject/body are prefilled from the form data.
- Message length is limited in the UI to keep broad compatibility with `mailto` URL limits.

There is no contact backend endpoint or server-side mail provider in this setup.
